...
impl Token {
    pub fn parse(token: &str) -> Result<Token> {
        ...
        Ok(Self {
            user: b64u_decode(user_b64u).map_err(|_| Error::TokenCannotDecodeUser)?,
...
/// generate a token for a given use identifier and its token salt
...
pub fn validate_token_sign_and_exp(origin_token: &Token, salt: &str) -> Result<()> {
    // -- validate signature
    let new_sign_b64u = token_sign_into_b64u(&origin_token.user, &origin_token.exp, salt)?;
...
/// create a token signature given the use identifier,
/// expiration and salt
fn token_sign_into_b64u(user: &str, exp: &str, salt: &str) -> Result<String> {
    let key = &config().TOKEN_KEY;
    let content = format!("{}.P{}", b64u_encode(user), b64u_encode(exp));
